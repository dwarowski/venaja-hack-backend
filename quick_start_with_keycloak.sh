...
fetch_jwks() {
    ...
    local REALM="todos"
    local JWKS_FILE_PATH="./jwks.json"
    ...
    echo -e "${YELLOW}Fetching JWKS from ${JWKS_URL} and saving to ${JWKS_FILE_PATH}...${NC}"
        ...
        curl -s http://localhost:8180/realms/todos/protocol/openid-connect/certs > files/jwks.json && cat ./jwks.json
...
echo -e "${YELLOW}Checking if the custom network 'quick_jwt_network' exists...${NC}"
if ! docker network ls | grep -q "quick_jwt_network"; then
    echo -e "${YELLOW}Creating custom network 'quick_jwt_network'...${NC}"
    docker network create quick_jwt_network || check_success "Failed to create custom network"
else
    echo -e "${GREEN}Custom network 'quick_jwt_network' already exists.${NC}"
fi

# Step 3: Remove conflicting containers if they exist
CONTAINERS=("postgres" "dataspace-builder" "dataspace-app" "keycloak-db" "keycloak")
...
cd ../files/resources/src-model/ || { echo -e "${RED}Error: Could not switch to ./files/resources/src-model/ directory.${NC}"; exit 1; }
